...
public class World4D {
	...
	/** Map of 0,1 values keyed by "x:y:z:w" */
	...
	public int getSurroundingActiveCount(int x, int y, int z, int w) {
		int sum = 0;
			...
			for (int y=minY-1; y<=maxY+1; y++) {
				for (int z=minZ-1; z<=maxZ+1; z++) {
					for (int w=minW-1; w<=maxW+1; w++) {
						...
						int state = get(x,y,z,w);
						int count = getSurroundingActiveCount(x,y,z,w);

						if (state == 1) {
							int newState = (count == 2 || count == 3) ? 1 : 0;
							world.set(x, y, z, w, newState);
						} else {
	...
	public static World4D loadFrom(File file) throws IOException {
		World4D world = new World4D();
	...
	public void printLayer(PrintStream out, int z, int w) {
		out.println("z = " + z + ", w = " + w);
